...
def get_number_of_lines():
    while True:
        lines = input('Enter the number of lines you like to bet on (1-' + str(MAX_LINES) + ")?")
        if lines.isdigit():
            lines = int(lines)
...
def spin(balance):
    ...
    while True:
        ...
        if total_bet > balance:
            print(f"You have insufficient funds. Your current balance is {balance}")
...
def main():
    ...
    while True:
        ...
        answer = input("Press enter to spin again (q to quit)")
        if answer == 'q':
            break 
